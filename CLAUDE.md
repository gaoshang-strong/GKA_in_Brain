...
| Step3_03 | ADMETlab **结果** | 每批 123 列 | ⏳ 1/16 批 |
| Step3_04 | 结果合并 | — | ⬜ 未做 |
...
对照的表现（**只是数据，判定属 Step3_05**）：
Fridén 按实测 Kp,uu 分段——`≥0.3` 组 18 个里 12 个判 BBB+；
`≤0.05` 组 14 个里 12 个判 BBB−；中间带 9 个对半开。
B3DB 对照——`control_positive` 351 个里 261 个判 Yes，`control_negative` 93 个里 72 个判 No。
...
- **`raw_smiles` 列让 ADMETlab 的行序风险降了一档**：它把输入原样返回，
  所以除了行序还能按结构独立校验（batch1 实测逐行一致、InChIKey 99/99 全同）。
